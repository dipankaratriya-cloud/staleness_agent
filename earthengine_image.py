...
flags.DEFINE_string(
    ...
    'Local directory to download the exported image.')
...
EE_DEFAULT_CONFIG = {
    ...
    'local_output_dir': _FLAGS.local_output_dir,
...
def ee_init(config: dict):
    ...
    if config.get('ee_remote'):
        ...
        ee.Initialize(credentials=credentials,
                      project=config.get('ee_gcloud_project') or project)
            ...
            for status in task_status:
                ...
                if gcs_path and image_file:
                    gcs_path = re.sub('https.*storage/browser/', 'gs://',
                                      gcs_path)
                    output_file = f'{gcs_path}{image_file}.tif'
                    status['output_file'] = output_file
                    if config.get('local_output_dir'):
                        local_file = os.path.join(config['local_output_dir'], f'{image_file}.tif')
                        logging.info(f'Downloading {output_file} to {local_file}')
                        file_util.file_copy(output_file, local_file)
